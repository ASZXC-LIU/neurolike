...
#### 2. 交互中枢 (Client Node) - 双端异构 (Dual-Client Strategy)

为了兼顾**开发效率**与**极致体验**，客户端采用双端并行策略，共享同一套后端协议。

##### A. 调试与Web端 (Vue Client)
- **定位**：**控制台与调试器 (Console & Debugger)**。
- **技术栈**：Vue 3 + TypeScript + Vite (Electron 可选)。
- **职责**：
  - **调试**：提供 `MonologueDebugger` (内心独白)、状态监控、参数热调。
  - **Web 体验**：在浏览器中运行，适合跨平台快速体验。
  - **开发**：快速验证新协议与 UI 逻辑。

##### B. 沉浸式桌宠端 (C++ Native Client)
- **定位**：**最终形态的桌面伴生体 (The Ultimate Desktop Pet)**。
- **技术栈**：C++ 17 + Live2D Cubism SDK Native + OpenGL/DirectX + Win32 API。
- **核心特性**：
  - **极致性能**：内存占用 < 100MB，CPU 占用忽略不计。
  - **真·透明窗口**：基于 `WS_EX_LAYERED` 与 DWM，实现完美的背景透明与抗锯齿。
  - **精准穿透 (Click-through)**：基于 Live2D 模型网格的精准点击检测。鼠标点在人物上拦截交互，点在空白处直接穿透操作桌面。
  - **系统级集成**：原生托盘、全局快捷键、开机自启。
...
#### 3. 主动意识与环境感知 (Active Awareness & Context Sensing)

打破“对讲机”模式，赋予 AI **“观察者”** 的能力。

- **视觉感知 (Visual Context)**：
  - **机制**：客户端定时（如每 5 秒）或在特定事件（如切换窗口）时，截取当前活动窗口的标题、进程名甚至屏幕截图（OCR）。
  - **应用**：当你打开 IDE 时，AI 自动切换到“编程助手”模式；当你打开 Steam 时，AI 吐槽“又摸鱼了？”。

- **系统节拍器 (System Tick)**：
  - **机制**：客户端维护一个后台 Cron 任务，每隔 N 分钟向总线广播 `CLIENT_SYSTEM_TICK`。
  - **Payload**：包含当前时间、用户空闲时长、前台应用、电池电量等。
  - **决策**：3060 收到 Tick 后，进行一次轻量级推理（Inner Monologue）。如果判定需要交互（如“深夜了”或“电量低”），则主动下发 TTS 语音。

- **无感交互 (Seamless Interaction)**：
  - **多模态输入**：除了语音，支持通过 IM 消息、文件拖拽、剪贴板变化触发 AI。
  - **静默陪伴**：在不打扰用户的前提下，通过 Live2D 的眼神跟随、小动作（伸懒腰、打瞌睡）来刷存在感，而不是一直说话。
  - **离线降级 (Offline Fallback)**：当 3060 服务端断连时，客户端自动切换至“离线行为树”模式，仅响应鼠标点击（播放本地预设语音），避免变成“僵尸”。

#### 4. 多端协同与互斥 (Multi-Client Coordination)

为了防止 Vue 调试端与 C++ 桌宠端同时播放音频导致“二重奏”，引入 **主控权抢占 (Master/Slave)** 机制。

- **机制**：同一时间只能有一个 Client 处于 `Active` (Master) 状态，负责音频播放与麦克风输入。其他 Client 自动降级为 `Passive` (Slave)，仅接收状态更新（如表情变化、日志），不发声。
- **策略**：
  - **默认**：C++ 桌宠端启动时自动抢占 Master。
  - **调试**：Vue 端可手动申请 Master 权限（用于测试音频）。
  - **互斥**：服务端维护 `current_master_id`，拒绝非 Master 节点的音频流请求。

#### 5. 客户端形态分离 (Client Separation)

Vue Client 和 C++ Client 是两个独立的应用程序，可以同时运行，各司其职。

- **Vue Client (Web/Electron)**：
  - **专注**：控制台、调试器、设置面板、聊天记录查看。
  - **Live2D**：可选开启。主要用于调试模型动作，或者在没有 C++ 客户端的环境下（如纯 Web）作为替代品。
  - **运行模式**：通常作为“控制台”运行，不负责主要的语音交互（除非手动抢占 Master）。

- **C++ Native Client**：
  - **专注**：桌面陪伴、语音交互、系统感知。
  - **Live2D**：核心功能，始终开启。
  - **运行模式**：常驻后台，作为主要的“躯体”存在。
...
- - ### 🛠️ NeuralLink 终极技术选型清单 (Tech Stack Blueprint)
    ...
    **定位**：纯粹的无头张量计算黑盒，提供“耳、脑、嘴”的推理能力。
    ...
    #### 二、 交互中枢 (Client Node - 双端并进)

    **策略**：Vue 端负责业务逻辑验证与调试，C++ 端负责最终用户体验。

    **1. Vue Client (调试与验证)**
    - **运行时**： Electron。
    - **技术栈**：Vue 3 + TypeScript + PixiJS (Live2D)。
    - **优势**：开发速度快，UI 丰富，适合做设置面板和调试器。

    **2. C++ Native Client (沉浸式桌宠)**
    - **运行时**：原生 Windows PE。
    - **技术栈**：C++ 17 + Cubism SDK + OpenGL + WebSocket++。
    - **优势**：
      - **透明窗口**：利用 DWM API 实现完美透明。
      - **交互穿透**：拦截 `WM_NCHITTEST` 实现像素级穿透。
      - **性能**：极低资源占用，常驻后台无感。
